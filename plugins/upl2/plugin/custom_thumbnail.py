...
from plugins.upl2.translation import Translation

import pyrogram
logging.getLogger("pyrogram").setLevel(logging.WARNING)


@pyrogram.Client.on_message(pyrogram.filters.command(["ghumbnail"]))
async def generate_custom_thumbnail(bot, update):
    if update.reply_to_message is not None:
        reply_message = update.reply_to_message
        if reply_message.media_group_id is not None:
            download_location = "downloads" + "/" + str(update.from_user.id) + "/" + str(reply_message.media_group_id) + "/"
...
async def save_photo(bot, update):
    if update.media_group_id is not None:
        # album is sent
        download_location = "downloads" + "/" + str(update.from_user.id) + "/" + str(update.media_group_id) + "/"
...
@pyrogram.Client.on_message(pyrogram.filters.command(["dhumbnail"]))
async def delete_thumbnail(bot, update):
    
    download_location = "downloads"+ "/" + str(update.from_user.id)
    try:
